...
class Pos_Bridge_Miscellaneous {
	...
	*/
     ...
     */
    public function oliver_pos_get_shop_amelia_details() {
		$order_data = array(
			'amelia_menus' => $this->oliver_pos_get_amelia_menu_list(),
		);
		return $order_data;
	}	
	public function oliver_pos_get_amelia_menu_list() {
	    $menu_data=array();
    ...
    public function oliver_pos_get_admin_username() {
	    $args = array(
			'role'    => 'administrator',
		...
		return empty($user_login) ? array() : $user_login; 
    }
	...
	public static function oliver_pos_get_oliver_setting() {
		return array(
			...
			'oliver_pos_subscription_email' => stripslashes( get_option('oliver_pos_subscription_email')),
	...
	public function oliver_pos_amelia_menu($menu_ord) {
        global $submenu;
		$amelia_menu='';
		if ( is_plugin_active( 'ameliabooking/ameliabooking.php' ) ){
			$amelia_menu = $submenu['amelia'];
			update_option('oliver_pos_amelia_menu', $amelia_menu);
		}
        /* wp_remote_post( esc_url_raw(ASP_TRIGGER_AMELIA_MENU), array(
            'headers'     => array('Content-Type' => 'application/json; charset=utf-8',
                'Authorization' => 'Basic ' . base64_encode( get_option( 'oliver_pos_subscription_email' ).":".get_option( 'oliver_pos_subscription_token' ))
            ),
            'body' => json_encode($amelia_menu),
        )); */
		return $menu_ord;
    }
}
