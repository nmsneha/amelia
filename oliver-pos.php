...
{
    if ( is_page( 'amelia' ) ) {
        $page_template = dirname( __FILE__ ) . '/templates/page-template.php';
		//var_dump($page_template);
    }
	if ( is_page( 'amelia seeting' ) ) {
        $page_template = dirname( __FILE__ ) . '/templates/amelia-setting.php';
		//var_dump($page_template);
    }
	if ( is_page( 'amelia submit form' ) ) {
        $page_template = dirname( __FILE__ ) . '/templates/amelia-submitform.php';
		//var_dump($page_template);
    }
...
{
    $templates[plugin_dir_path(__FILE__) . 'templates/page-template.php'] = __('Amelia Template', 'text-domain');
	$templates[plugin_dir_path(__FILE__) . 'templates/amelia-setting.php'] = __('Amelia setting', 'text-domain');
	
	$templates[plugin_dir_path(__FILE__) . 'templates/amelia-submitform.php'] = __('Amelia Submit', 'text-domain');
...
$posttitle = $my_posts[0]->post_title;	
		...
		/* if ( ! empty($posttitle)) {
				...
				update_post_meta($id, '_wp_page_template', plugin_dir_path(__FILE__) . 'templates/page-template.php');
		} */

update_option('hibye', $templates);
    return $templates;
}
...
// hide menu and unset cooking of open without iframe
$hide_menu = isset( $_COOKIE['show_amelia_only'] ) ? $_COOKIE['show_amelia_only'] : '';

if(!empty($hide_menu)){
	add_action( 'init', 'my_remove_menu_pages' );
...
if(@$all_headers['Sec-Fetch-Dest'] == "iframe"){
    //echo 'i am from iframe';
}
else{
	//echo 'outside iframe';
	/* if(isset( $_COOKIE['show_amelia_only'] )){
		//unset($_COOKIE['show_amelia_only']);
		//setcookie('show_amelia_only', null, -1, '/');	
		$cst_base_options = [
			'expires' => time() - 3600,
		...
		unset_cookie_with_samesite( 'show_amelia_only', '', $cst_base_options + ['secure' => true, 'path' => '/']);		
  ...
  ?>
   <style>
		...
		.notice, .updated, .toplevel_page_oliver-pos .notice-error, .error, .is-dismissible{
...
function unset_cookie_with_samesite($name, $value, array $options) {
	$header = 'Set-Cookie:';
...
add_shortcode('check_product', 'check_product');
